...
public class QuestionServiceImpl implements QuestionService {
	...
	@Override
	public List<Text> evaluateQuestion(int userId, int questionId, int mark) throws ServiceException {

		DaoFactory daoInstance = DaoFactory.getInstance();
		QuestionDao questionDao = daoInstance.getQuestionDao();

		try {
			List<Text> textList = questionDao.evaluateQuestion(userId, questionId, mark);
			return textList;
		} catch (DaoException ex) {
			throw new ServiceException(ex);
		}
