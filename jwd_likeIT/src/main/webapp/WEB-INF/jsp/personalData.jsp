...
<fmt:formatDate value="${sessionScope.current_user.birthday}"
	pattern="yyyy-MM-dd" var="user_birthday" />
	...
	<form action="${pageContext.request.contextPath}/Controller"
		...
		<div class="personal-info">
			...
			<label><c:out value="${surname}" /> <br> <input
				type="text" name="surname" pattern="^[\D]+$"
				value="<c:out value="${sessionScope.current_user.surname}" />"
				required> </label> <br> <label><c:out value="${name}" />
				<br> <input type="text" name="name" pattern="^[\D]+$"
				value="<c:out value="${sessionScope.current_user.name}" />" required>
			</label> <br> <label><c:out value="${birthday}" />* <br> <input
				type="date" name="date" min="1900-01-01" max="2011-01-01"
				value="<c:out value="${user_birthday}" />"></label> <br> <label><c:out
					value="${status}" /> <br> <select name="status" id="status">
					<option value="none" ${user_status == 'none' ? 'selected' : ''}>------</option>
					<option value="student"
						${user_status == 'student' ? 'selected' : ''}><c:out
							value="${student}" /></option>
					<option value="trainer"
						${user_status == 'trainer' ? 'selected' : ''}><c:out
							value="${trainer}" /></option>
					<option value="guru" ${user_status == 'guru' ? 'selected' : ''}><c:out
							value="${guru}" /></option>
			</select> </label> <br> <label><c:out value="${email}" />* <br> <input
				type="email" name="email" placeholder="user@example.com"
				pattern="^.+@.+\.[a-zA-Z]+$" id="first-email"
				value="<c:out value="${user_emails[0]}" />" required> </label><br>
			<label><c:out value="${email_2}" /> <br> <input
				type="email" name="second-email" placeholder="user@example.com"
				pattern="^.+@.+\.[a-zA-Z]+$" id="second-email"
				value="<c:out value="${user_emails[1]}" />"> </label><br>
		</div>
